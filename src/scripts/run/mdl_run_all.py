...
# 1. 运行单个seed实验
python src/scripts/run/mdl_run_all.py all --seed 42 --use-meta --use-attr

# 2. 运行all模式seed区间实验
python src/scripts/run/mdl_run_all.py all --start-seed 42 --end-seed 46 --no-use-meta --use-attr

# 3. 运行single模式seed区间实验
python src/scripts/run/mdl_run_all.py single --start-seed 42 --end-seed 46 --no-use-meta --use-attr
"""
...
def _build_command(seed: int, device: str, use_meta: bool, use_attr: bool) -> CommandItem:
    ...
    exp_name = f"MDL: clip image+text+{_meta_label(use_meta)}+{_attr_label(use_attr)} (seed {seed})"
    return cmd, exp_name
...
def run_all_experiments(args: argparse.Namespace) -> None:
    ...
    if args.mode == "all":
        ...
        _run_command_group(
            all_commands,
            (
                f"MDL实验 ({_format_seed_range_label(start_seed, end_seed)}, "
        ...
        _run_command_group(
            single_commands,
            (
                f"MDL seeds {args.start_seed}-{args.end_seed} "
                f"({_meta_label(args.use_meta)}, {_attr_label(args.use_attr)})"
            ),
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="运行MDL模型实验脚本")
    ...
    print(f"一键运行MDL实验脚本 (模式: {args.mode})")
    print("=" * 50)
