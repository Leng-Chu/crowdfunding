...
def _build_command(seed: int, device: str, experiment_mode: str) -> CommandItem:
    ...
    mode_to_name = {
        ...
        "image_text": "MDL: clip image+text",
    }
...
def run_all_experiments(args: argparse.Namespace) -> None:
    ...
    if args.mode == "all":
        ...
        _run_command_group(
            single_commands,
            f"MDL {args.experiment_mode}, seeds {args.start_seed}-{args.end_seed}",
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="运行 MDL 模型实验脚本")
    ...
    print(f"一键运行 MDL 实验脚本 (模式: {args.mode})")
    print("=" * 50)
