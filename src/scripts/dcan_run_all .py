...
def run_all_experiments():
    ...
    all_commands = [
        # meta + image + text
        ("conda run -n crowdfunding python src/dl/dcan/main.py --run-name clip --image-embedding-type clip --text-embedding-type clip --use-meta --device cuda:0", 
         "Multi-modal: CLIP Image+Text with Meta"),
        ("conda run -n crowdfunding python src/dl/dcan/main.py --run-name siglip --image-embedding-type siglip --text-embedding-type siglip --use-meta --device cuda:1", 
         "Multi-modal: SigLIP Image+Text with Meta"),
        ("conda run -n crowdfunding python src/dl/dcan/main.py --run-name bge-clip --image-embedding-type clip --text-embedding-type bge --use-meta --device cuda:2", 
         "Multi-modal: CLIP Image + BGE Text with Meta"),
        ("conda run -n crowdfunding python src/dl/dcan/main.py --run-name bge-siglip --image-embedding-type siglip --text-embedding-type bge --use-meta --device cuda:3", 
         "Multi-modal: SigLIP Image + BGE Text with Meta"),
        
        # image + text
        ("conda run -n crowdfunding python src/dl/dcan/main.py --run-name clip --image-embedding-type clip --text-embedding-type clip --no-use-meta --device cuda:0", 
         "Image+Text: CLIP"),
        ("conda run -n crowdfunding python src/dl/dcan/main.py --run-name siglip --image-embedding-type siglip --text-embedding-type siglip --no-use-meta --device cuda:1", 
         "Image+Text: SigLIP"),
