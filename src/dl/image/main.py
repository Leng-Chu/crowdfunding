...
- 从 data/projects/.../<project_id>/ 读取图片嵌入（cover + image）
- 训练 PyTorch MLP（二分类）
- 保存产物到 experiments/image_dl/<run_id>/{artifacts,reports,plots}

运行示例（在项目根目录）：
conda run -n crowdfunding python src/dl/image/main.py
"""
