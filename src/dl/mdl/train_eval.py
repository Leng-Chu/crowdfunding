...
- 损失：BCEWithLogitsLoss（带轻量 label smoothing）
- 优化器：AdamW
- 学习率调度：warmup + cosine（每 step 更新）
- AMP：仅 CUDA 启用
- EMA：评估/early stopping 使用 EMA 权重
...
def _get_device(cfg: MdlConfig) -> torch.device:
...
def train_multimodal_with_early_stopping(
    ...
    best_info = {
        ...
        "best_val_auc": None
        if metric_for_best != "val_auc"
        else (None if not np.isfinite(best_val_auc) else float(best_val_auc)),
