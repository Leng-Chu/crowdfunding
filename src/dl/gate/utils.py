...
def compute_binary_metrics(y_true: np.ndarray, y_prob: np.ndarray, threshold: float = 0.5) -> Dict[str, Any]:
    ...
    logloss = (
        float(-np.mean(y_true_1d * np.log(y_prob_1d) + (1 - y_true_1d) * np.log(1 - y_prob_1d)))
        if n > 0
        else 0.0
    )
...
def plot_history(history: List[Dict[str, Any]], save_path: Path) -> None:
    ...
    x1, y1 = _extract_xy("train_log_loss")
    x2, y2 = _extract_xy("val_log_loss")
    if x1 and y1:
        ax.plot(x1, y1, label="train")
    if x2 and y2:
        ax.plot(x2, y2, label="val")
    ...
    x3, y3 = _extract_xy("train_roc_auc")
    x4, y4 = _extract_xy("val_roc_auc")
    if x3 and y3:
        ax2.plot(x3, y3, label="train")
    if x4 and y4:
        ax2.plot(x4, y4, label="val")
