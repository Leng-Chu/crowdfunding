# -*- coding: utf-8 -*-
"""
gate 主程序入口（Chapter 2：三分支 + Two-stage gated fusion）：

说明：
- 本目录代码可独立运行，不得 import 或复用 `src/dl/seq` 的代码
- 工程行为与输出结构对齐 seq，便于横向对比
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="gate 训练入口（支持命令行覆盖少量常用配置）。")
...
def main() -> int:
    ...
    # 先 import torch，可避免部分环境出现 DLL 初始化失败（对齐 seq/mlp 的做法）。
    import torch  # noqa: F401

    from data import prepare_gate_data
    ...
    # 产物目录命名：use_meta 已移除；除 seq_only/key_only 外默认都使用 meta，因此目录直接用 baseline_mode。
    mode = baseline_mode
    ...
    run_dir_file = str(os.getenv("GATE_RUN_DIR_FILE", "") or "").strip()
    if run_dir_file:
        try:
            p = Path(run_dir_file)
            ...
            print(f"写入 GATE_RUN_DIR_FILE 失败：{e}", file=sys.stderr)
    ...
    head_hidden_dim = int(getattr(model, "head_hidden_dim", 0)) or None
    if head_hidden_dim is not None:
        logger.info("head_hidden_dim（自动）：%d", int(head_hidden_dim))
    ...
    save_json(
        {
            ...
            "head_hidden_dim": None if head_hidden_dim is None else int(head_hidden_dim),
            **cfg.to_dict(),
        },
        reports_dir / "config.json",
    )

    best_state, best_epoch, history, best_info = train_gate_with_early_stopping(
        model,
    ...
    torch.save(
        {
            ...
            "head_hidden_dim": None if head_hidden_dim is None else int(head_hidden_dim),
            "head_dropout": float(getattr(cfg, "head_dropout", 0.0)),
