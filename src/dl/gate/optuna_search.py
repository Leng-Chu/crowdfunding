...
def _suggest_params(trial, baseline_mode: str) -> Dict[str, Any]:
    """
    默认搜索空间（偏向 two_stage，但也可用于其它 baseline）。
    注意：这里返回的是 GateConfig 字段名/值，最终会通过环境变量覆盖到训练脚本。
    """
...
def _arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="gate Optuna 超参搜索（每个 trial 调用现有 CLI 黑盒训练）。")
    p.add_argument(
        "--baseline-mode",
        default="two_stage",
        choices=["late_concat", "stage1_only", "stage2_only", "two_stage", "seq_only", "key_only"],
        help="要调参的 baseline_mode（默认 two_stage）。",
    )
    ...
    p.add_argument("--fixed-overrides", default=None, help="对所有 trial 生效的 GateConfig 覆盖项：JSON 字符串或 JSON 文件路径")
...
def main() -> int:
    ...
    study_name = (
        ...
        else f"gate_{baseline_mode}_{objective_col}"
    )
    root = _project_root()
    study_dir = root / "experiments" / "gate" / "optuna" / study_name
    ...
    fixed_overrides["debug_gate_stats"] = False
    ...
    def objective(trial) -> float:
        ...
        trial_params = _suggest_params(trial, baseline_mode=baseline_mode)
        overrides = dict(fixed_overrides)
        overrides.update(trial_params)

        env = dict(os.environ)
        env["GATE_CFG_OVERRIDES"] = json.dumps(overrides, ensure_ascii=False)
        env["GATE_RUN_DIR_FILE"] = str(run_dir_file)
        env["PYTHONUTF8"] = "1"
        ...
        if status == "ok" and run_dir is not None and run_dir.exists():
            ...
            if run_dir is None:
                error_msg = "未获取到 run_dir（GATE_RUN_DIR_FILE 为空或不存在）。"
