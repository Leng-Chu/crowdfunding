# -*- coding: utf-8 -*-
"""
环境变量覆盖 GateConfig（供自动化脚本/Optuna 调参使用）。
...
用法示例（bash）：
  export GATE_CFG_OVERRIDES='{"learning_rate_init": 0.0002, "batch_size": 512, "random_seed": 42}'
  python src/dl/gate/main.py --baseline-mode two_stage
"""
...
ENV_CFG_OVERRIDES = "GATE_CFG_OVERRIDES"
...
def apply_config_overrides_from_env(cfg: GateConfig, logger=None) -> GateConfig:
    """
    从环境变量读取配置覆盖项并返回新 cfg。

    环境变量：
    - GATE_CFG_OVERRIDES：JSON 字典，例如 {"learning_rate_init": 1e-4, "batch_size": 512}
    """
