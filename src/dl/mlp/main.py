...
def main() -> int:
    ...
    from data import iter_multimodal_kfold_data, prepare_multimodal_data
    ...
    if split_mode == "kfold":
        ...
        for fold_idx, prepared in iter_multimodal_kfold_data(
            ...
            if prepared.stats:
                logger.info("fold=%d 统计信息：%s", int(fold_idx), prepared.stats)
            ...
            try:
                ...
                logger.warning("fold=%d 保存 splits.csv 失败：%s", int(fold_idx), e)
            ...
            try:
                ...
                logger.warning("fold=%d 保存 history.csv 失败：%s", int(fold_idx), e)
            ...
            save_json(fold_result, fold_reports_dir / "metrics.json")
