# -*- coding: utf-8 -*-
"""
环境变量覆盖 SeqConfig（供自动化脚本/Optuna 调参使用）。
...
用法示例（PowerShell）：
  $env:SEQ_CFG_OVERRIDES='{\"learning_rate_init\": 0.0002, \"batch_size\": 256, \"random_seed\": 42}'
  python src/dl/seq/main.py --baseline-mode trm_pos --use-meta
"""
...
ENV_CFG_OVERRIDES = "SEQ_CFG_OVERRIDES"
...
def apply_config_overrides_from_env(cfg: SeqConfig, logger=None) -> SeqConfig:
    """
    从环境变量读取配置覆盖项并返回新 cfg。

    环境变量：
    - SEQ_CFG_OVERRIDES：JSON 字典，例如 {"learning_rate_init": 1e-4, "batch_size": 512}
    """
