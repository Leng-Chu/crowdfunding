...
- 固定使用 image + text 两路输入
- 可选 meta：仅在融合阶段 concat，不参与 DCAN 注意力交互
"""
...
def _get_device(cfg: DcanConfig) -> torch.device:
...
def _make_mask_from_lengths(lengths: torch.Tensor, max_len: int) -> torch.Tensor:
    """lengths: [B] -> mask: [B, L]，True 表示有效位置。"""
    if lengths.ndim != 1:
        lengths = lengths.reshape(-1)
    arange = torch.arange(int(max_len), device=lengths.device).unsqueeze(0)  # [1, L]
    return arange < lengths.unsqueeze(1)


@torch.no_grad()
def _positive_proba_dcan(
    model: nn.Module,
    use_meta: bool,
    ...
    x_img = torch.from_numpy(np.asarray(X_image, dtype=np.float32))
    len_img = torch.from_numpy(np.asarray(len_image, dtype=np.int64))
    x_txt = torch.from_numpy(np.asarray(X_text, dtype=np.float32))
    len_txt = torch.from_numpy(np.asarray(len_text, dtype=np.int64))

    if bool(use_meta):
        if X_meta is None:
            raise ValueError("use_meta=True 但 X_meta 为空。")
        x_meta = torch.from_numpy(np.asarray(X_meta, dtype=np.float32))
        dataset = TensorDataset(x_meta, x_img, len_img, x_txt, len_txt)
    else:
        dataset = TensorDataset(x_img, len_img, x_txt, len_txt)
    ...
    for batch in loader:
        if bool(use_meta):
            xb_meta, xb_img, lb_img, xb_txt, lb_txt = batch
            xb_meta = xb_meta.to(device)
        else:
            xb_img, lb_img, xb_txt, lb_txt = batch
            xb_meta = None

        xb_img = xb_img.to(device)
        lb_img = lb_img.to(device)
        xb_txt = xb_txt.to(device)
        lb_txt = lb_txt.to(device)

        img_mask = _make_mask_from_lengths(lb_img, max_len=int(xb_img.shape[1]))
        txt_mask = _make_mask_from_lengths(lb_txt, max_len=int(xb_txt.shape[1]))
...
def train_dcan_with_early_stopping(
    model: nn.Module,
    use_meta: bool,
    ...
    x_img = torch.from_numpy(np.asarray(X_image_train, dtype=np.float32))
    len_img = torch.from_numpy(np.asarray(len_image_train, dtype=np.int64))
    x_txt = torch.from_numpy(np.asarray(X_text_train, dtype=np.float32))
    len_txt = torch.from_numpy(np.asarray(len_text_train, dtype=np.int64))
    y_train_t = torch.from_numpy(np.asarray(y_train, dtype=np.float32))

    if bool(use_meta):
        if X_meta_train is None:
            raise ValueError("use_meta=True 但 X_meta_train 为空。")
        x_meta = torch.from_numpy(np.asarray(X_meta_train, dtype=np.float32))
        train_loader = DataLoader(
            TensorDataset(x_meta, x_img, len_img, x_txt, len_txt, y_train_t),
            batch_size=max(1, int(cfg.batch_size)),
            shuffle=True,
        )
    else:
        train_loader = DataLoader(
            TensorDataset(x_img, len_img, x_txt, len_txt, y_train_t),
            batch_size=max(1, int(cfg.batch_size)),
            shuffle=True,
        )
    ...
    for epoch in range(1, int(cfg.max_epochs) + 1):
        ...
        for batch in train_loader:
            if bool(use_meta):
                xb_meta, xb_img, lb_img, xb_txt, lb_txt, yb = batch
                xb_meta = xb_meta.to(device)
            else:
                xb_img, lb_img, xb_txt, lb_txt, yb = batch
                xb_meta = None
        ...
        train_prob = _positive_proba_dcan(
            model,
            use_meta=use_meta,
            X_meta=X_meta_train,
