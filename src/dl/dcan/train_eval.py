...
- 固定使用 image + text 双路输入
- 可选 meta：仅在融合阶段 concat，不参与 DCAN 注意力交互
- 与 late 对齐：
  - best checkpoint：优先 val_auc；若验证集单类导致 AUC 不可用，回退 val_log_loss
  - 训练技巧：label smoothing / EMA / AMP
  - 学习率更新：warmup + cosine（按 step 更新）
"""
...
def _build_grad_scaler(enabled: bool):
    ...
    说明：仅在 CUDA 设备上启用 AMP，CPU 上返回禁用状态的 scaler。
    """
    if bool(enabled):
...
def _make_mask_from_lengths(lengths: torch.Tensor, max_len: int) -> torch.Tensor:
    ...
    arange = torch.arange(int(max_len), device=lengths.device).unsqueeze(0)
