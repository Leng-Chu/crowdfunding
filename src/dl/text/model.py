# -*- coding: utf-8 -*-
"""
模型定义模块：使用 PyTorch 1D CNN 做二分类（与给定示意图对齐）。

输入：
- 堆叠后的文本嵌入序列，形状为 (B, L, D)
  - L：向量条数（title_blurb=1~2；若存在 text_{type}.npy 则 L=Lt+N）
  - D：嵌入维度（随 bge/clip/siglip 而定）
...
class TextCNNBinaryClassifier(nn.Module):
    ...
    def _init_weights(self) -> None:
        """初始化（conv 用 kaiming；linear 用 xavier）。"""
...
def build_cnn(cfg: TextDLConfig, embedding_dim: int) -> TextCNNBinaryClassifier:
